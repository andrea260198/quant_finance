...
class TestBalanceSheetInitalizer(unittest.TestCase):
    def test_simple_example(self):
        ...
        contracts = [
            EuropeanCallOption(strike=100, T=5, r=np.nan, S_0=np.nan, sigma=np.nan, currency=ISO4217("USD"), underlying_quantity=1, underlying_name="AAPL"),
            EuropeanPutOption(strike=100, T=5, r=np.nan, S_0=np.nan, sigma=np.nan, currency=ISO4217("USD"), underlying_quantity=1, underlying_name="GOOG"),
        ]
        initialized_contracts = BalanceSheetInitalizer(contracts=contracts, scenary_cube=scenary_cube).run()
